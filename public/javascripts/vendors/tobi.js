...
(function (root, factory) {
  ...
  var Tobi = function Tobi (userOptions) {
   ...
   if (document.getElementsByClassName('tobi-overlay').length) {
     var overlay = document.getElementsByClassName('tobi-overlay')[0];
     overlay.innerHTML = '';
   } else {
    ...
    prevButton.onclick = () => tobi.prev();
    overlay.appendChild(prevButton)

    var nextButton = document.createElement('button')
    nextButton.setAttribute('type', 'button')
    nextButton.onclick = () => tobi.next();
    ...
    var initElement = function initElement (element) {
      if (gallery.indexOf(element) === -1) {
        ...
        element.addEventListener('click', function (event) {
          ...
          if (element.dataset.href === '#lightbox-video-1') {
            document.getElementsByClassName('tobi-video')[1].play();
          } else if (element.dataset.href === '#lightbox-audio-1') {
            document.getElementsByClassName('tobi-audio')[1].play();
          }
          if (tobi) {
            return tobi.open(element.dataset.lightboxindex);
          }
          openOverlay(element.dataset.lightboxindex);
        })
    ...
    var clickHandler = function clickHandler (event) {
      if (event.target === closeButton || (event.target.classList.contains('tobi-figure-wrapper') && !event.target.classList.contains('tobi-figure'))) {
        closeOverlay()
      }
